...
<div class="_hidden_inputs _filters _tasks_filters">
    ...
    if(isset($project->id)) 
    {
        $this->db->select('tblmilestones.name, tblmilestones.id');
        $this->db->from('tbltasks');
        $this->db->join('tblmilestones' ,'tblmilestones.id=tbltasks.milestone', 'INNER');
        $this->db->where('tbltasks.rel_id', $project->id);
        $this->db->where('tbltasks.rel_type', 'project');
        $this->db->group_by('tbltasks.milestone');
        $query = $this->db->get();
...
<div class="btn-group pull-right mleft4 btn-with-tooltip-group _filter_data" data-toggle="tooltip"
    ...
    <?php if (has_permission('tasks', '', 'view')) { ?>
        <?php if (isset($sprints) && count($sprints)) { ?>
        <li class="dropdown-submenu pull-left">
            <a href="#" tabindex="-1"><?php echo _l('filter_by_sprint'); ?></a>
            <ul class="dropdown-menu dropdown-menu-left">
                <?php foreach ($sprints as $sprint) { ?>
                <li>
                    <a href="#" data-cview="task_sprint_<?php echo $sprint['id']; ?>" onclick="dt_custom_view(<?php echo $sprint['id']; ?>,'<?php echo $view_table_name; ?>','task_sprint'); return false;">
